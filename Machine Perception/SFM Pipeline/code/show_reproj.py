...
def show_reprojections(image1, image2, uncalibrated_1, uncalibrated_2, P1, P2, K, T, R, plot=True):
  ...
  """ YOUR CODE HERE
  """
  P1 = np.hstack((P1, np.ones_like(P1[:,0].reshape(-1,1))))

  P2 = np.hstack((P2, np.ones_like(P2[:,0].reshape(-1,1))))
  
  extrinsic = np.column_stack((R,T))

  P1proj = (K@extrinsic@P1.T).T
  # print(P1proj.shape)
  t = -R.T@T.reshape(-1,1)
  # print(t.shape)
  extrinsic = np.column_stack((R.T,t))
  P2proj = (K@extrinsic@P2.T).T
  # print(P2proj.shape)
