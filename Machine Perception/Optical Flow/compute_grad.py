...
def compute_Ix(imgs):
    ...
    ### STUDENT CODE START ###
    Ix = convolve1d(imgs, KERNEL_H, axis = 1) #x
    Ix = convolve1d(Ix, KERNEL_G, axis= 0) #y
    Ix = convolve1d(Ix, KERNEL_G , axis = 2)#time
    # plt.imshow(Ix[:, :, 0])
    # plt.show()
    ### STUDENT CODE END ###
...
def compute_Iy(imgs):
    ...
    Iy = convolve1d(imgs, KERNEL_H, axis = 0) #x
    Iy = convolve1d(Iy, KERNEL_G, axis= 1) #y
    Iy = convolve1d(Iy, KERNEL_G , axis = 2)#time
