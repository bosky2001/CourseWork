...
class Controller2D(object):
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            #Heading Error
            traj_yaw = np.arctan2(waypoints[-1][1]-waypoints[0][1],waypoints[-1][0]-waypoints[0][0])
            delta_yaw = traj_yaw -yaw
            ...
            #Cross track error
            state=np.array([x,y])
            error =np.min(np.sum((state - np.array(waypoints)[:, :2]) ** 2, axis=1))

            path_yaw = traj_yaw - np.arctan2(y - waypoints[0][1], x - waypoints[0][0])
            ...
            delta_ct = np.arctan(e_k*error/(v_k+v))

            steer_expect = delta_ct+ delta_yaw
