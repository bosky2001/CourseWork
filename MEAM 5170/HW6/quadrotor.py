...
class Quadrotor(object):
  ...
  def add_dynamics_constraint(self, prog, x, u, N, T):
    ...
    for i in range (N-1):
        prog.AddLinearEqualityConstraint(x[i+1]-A@x[i]-B@u[i],np.zeros(6))
    

  def add_cost(self, prog, x, u, N):
    # TODO: add cost.
    for i in range (N-1):
        prog.AddQuadraticCost(u[i]@self.R @ u[i].T + x[i] @ self.Q @ x[i].T)
  ...
  def compute_mpc_feedback(self, x_current, use_clf=False):
    ...
    u_d= np.array([self.m*self.g/2, self.m*self.g/2])
    u_mpc = result.GetSolution(u[0])+u_d
