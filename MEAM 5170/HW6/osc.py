...
class OperationalSpaceWalkingController(LeafSystem):
    ...
    def SetupAndSolveQP(self,  context: Context) -> Tuple[np.ndarray, MathematicalProgram]:
        ...
        for traj_name in self.trajs:
            ...
            b=yddot_cmd_i-JdotV_i
            prog.AddQuadraticCost(2*J_i.T@ W_i@J_i ,-2*J_i.T @ W_i@ b, b.T @W_i@ b,vdot,is_convex=True)

        # TODO: Add Quadratic Cost on vdot using self.gains.w_vdot * Identity
        prog.AddQuadraticCost(2*self.gains.w_vdot*np.eye(self.plant.num_velocities()),np.zeros((self.plant.num_velocities(),)),vdot,is_convex=True)
        ...
        prog.AddLinearEqualityConstraint(M @vdot + Cv+G -B@u-J_c.T @lambda_c,np.zeros_like(G))
        # TODO: Add Contact Constraint
        prog.AddLinearEqualityConstraint(J_c@vdot +J_c_dot_v, np.zeros_like(J_c_dot_v))
