...
def find_throwing_trajectory(N, initial_state, final_configuration, distance, tf):
  ...
  prog.AddLinearEqualityConstraint(xf[:2], final_configuration)
  for i in range (N):
    prog.AddBoundingBoxConstraint(-joint_limits,joint_limits,x[i,:2])
    prog.AddBoundingBoxConstraint(-vel_limits,vel_limits,x[i,-2:])
  ...
  for i in range(N):
      uguess[i,:]=-1*effort_limits.reshape((1,-1))
  uguess = uguess +np.random.random_sample((N,n_u))*2
  prog.SetInitialGuess(u,uguess)

  xguess=np.zeros((N,n_x))
