...
def add_continuity_constraints(prog, sigma, n, d, dt):
  ...
  for i in range(n - 1):
    for k in range(1, 5):
      constraint_expr = -factorial(k) * sigma[i+1, k]
      for j in range(k, d):
        constraint_expr += factorial(j) / factorial(j - k) * sigma[i,j] * dt[i] ** (j-k)
      prog.AddLinearEqualityConstraint(constraint_expr, np.zeros(2))
...
def add_minsnap_cost(prog, sigma, n, d, dt):
  ...
  for i in range(n):
    for k in range(4, d):
      for l in range(4, d):
        ex = k+l-7
        cost_expr = (factorial(k)/factorial(k-4)) * (factorial(l)/factorial(l-4))*(dt[i]**ex)/(ex) * sigma[i,k] @ sigma[i,l]
