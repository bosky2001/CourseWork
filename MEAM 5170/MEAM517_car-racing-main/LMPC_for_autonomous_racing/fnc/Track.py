...
class Map():
    ...
    def __init__(self, halfWidth=0.4):
        ...
        # the first col is segment length, 
        # the second col is curve radius(+ for left hand turn, - for right hand, 0 for straight line)
        '''track 1'''
        #spec = np.array([[2.0, 0],
        ...
        # These two are not able to be optimized by LMPC!!
        """
        spec = np.array([[1.0, 0],
                         [lengthCurve, lengthCurve / np.pi],
                         # Note s = 1 * np.pi / 2 and r = -1 ---> Angle spanned = np.pi / 2
        ...
        plt.plot(self.PointAndTangent[:, 0], self.PointAndTangent[:, 1], 'or')
        # plt.show()
