...
class MPCSim:
    def __init__(self):
        ...
        # Cost Matrices
        Q = np.diag([7, 7, 0, 5])  # state error cost
        Qf = np.diag([30, 30, 30, 30])  # state final error cost
        R = np.diag([1, 1])  # input cost
        R_ = np.diag([1, 1])  # input rate of change cost
    ...
    def run(self):
        ...
        while 1:
            ...
            # )
            target, _ = mpcpy.get_ref_trajectory(
                self.state, self.path, VEL, dl=0.3
            )
            # target, _ = mpcpy.get_ref_trajectory(
            #     self.state, self.path, VEL, dl=0.1
            # )
...
if __name__ == "__main__":
    ...
    trackLineUpdata = np.load('TracklineUp.npy')
    firstrow = trackLineUpdata[:, 0].reshape((192,))
    secondrow = trackLineUpdata[:, 1].reshape((192,))
    trackLineUp = np.vstack((firstrow, secondrow))

    trackLineLowdata = np.load('TracklineLow.npy')
    firstrow = trackLineLowdata[:, 0].reshape((192,))
    secondrow = trackLineLowdata[:, 1].reshape((192,))
    trackLineLow = np.vstack((firstrow, secondrow))



    #data_array = np.load('traj.npy')
    #path = data_array.reshape((2,68))

    data_array = np.load('centerline.npy')                             #Simulation RUn with centerline
    firstrow = data_array[:, 0].reshape((192,))
    secondrow = data_array[:, 1].reshape((192,))
    newfirstrow = np.zeros((65,))
    newsecondrow = np.zeros((65,))
    for idx in range(64):
        newfirstrow[idx] = firstrow[idx*3]
        newsecondrow[idx] = secondrow[idx*3]
    newfirstrow[64] = firstrow[191]
    newsecondrow[64] = secondrow[191]

    path = np.vstack((newfirstrow, newsecondrow))

    dx = np.append(0, np.diff(path[0]))
    dy = np.append(0, np.diff(path[1]))
    theta = np.arctan2(dy, dx)
    #globalPath = np.vstack((path,theta.reshape((1,68))))
    globalPath = np.vstack((path,theta.reshape((1,65))))                  # for centerline run this instead

    SIM_START_X = globalPath[0,0]
    SIM_START_Y = globalPath[1,0]
